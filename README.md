...
python3 scripts/operator_client.py <ip-de-um-c2>:9002 10.20.0.10 8080 10 15
```

> **Windows / macOS (Podman Desktop):** o Podman roda dentro de uma VM (via
> WSL2 no Windows), e a rede `10.20.0.0/24` do laboratório só existe *dentro*
> dessa VM — não é alcançável diretamente do host. `scripts/operator_client.py`
> precisa rodar de dentro de um container já conectado à rede:
> ```powershell
> podman cp scripts\operator_client.py ddos-lab_c2-a_1:/tmp/operator_client.py
> podman exec ddos-lab_c2-a_1 python /tmp/operator_client.py 10.20.0.23:9002 10.20.0.10 8080 10 20
> ```
> Em Linux com Podman nativo (sem VM), o host enxerga a rede diretamente e o
> comando original acima, direto do host, já funciona sem esse passo extra.
